...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    erased_height.y = (frame_bb.Max.y - frame_bb.Min.y) * 0.005f * (165.0f - g.Style.SliderHThickness);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    ImVec2 erased_width = ImVec2(0.0f, 0.0f);
    erased_width.x = (frame_bb.Max.x - frame_bb.Min.x) * 0.005f * (100.0f - g.Style.SliderVThickness);
    RenderFrame(frame_bb.Min + erased_width, frame_bb.Max - erased_width, frame_col, true, g.Style.FrameRounding);
